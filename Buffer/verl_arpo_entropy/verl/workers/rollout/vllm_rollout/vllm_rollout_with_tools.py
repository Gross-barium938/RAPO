    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**kwargs):
            ...
            while active_indices:
                ...
                entropy_norm_factor = math.log(vocab_size) # 归一化参数
                ...
                for i, out_idx in enumerate(active_indices):
                    # print(f"Now processing out_idx: {out_idx}")
                    output = outputs[i]
                    generated_tokens = output.outputs[0].token_ids

                    step_input_ids = curr_inputs[out_idx].copy()

                    step_input_text = self.tokenizer.decode(curr_inputs[out_idx], skip_special_tokens=True)
                    ...
                    step_output_text = decoded_text

                    if out_idx not in rollout_traces:
                        rollout_traces[out_idx] = []
                    rollout_traces[out_idx].append({
                        "key": step_input_text,
                        "value": step_output_text
                    })
                    trace_list.append(out_idx)

                    last_trace_index[out_idx] = len(rollout_traces[out_idx]) - 1
                    ...
                    for future in concurrent.futures.as_completed(futures):
                        ...
                        try:
                            ...
                            if not result_text:
                                ...
                                logger.debug(f"Tool({tag}) result:  Successful, result masked!") # {result_text}")
                        ...
                        curr_inputs[idx].extend(result_tokens) # 加在后面
                        result_masks[idx].extend([0] * len(result_tokens)) # 这些并不是policy生成的东西

                        trace_idx = last_trace_index[idx]
                        rollout_traces[idx][trace_idx]["value"] += f"{formatted_result}"
                ...
                for idx in final_active_indices:
                    ...
                    orig_sample = None
                    for sample_idx, indices in sample_to_indices.items(): # {idex: [1, 2, 3]}
                        if idx in indices:
                ...
                for orig_sample, active_idxs in active_by_sample.items():
                    ...
                    for source_idx in active_idxs:
                        branches_per_idx = min(beam_size - 1, remaining_slots - branches_created) # 1
                        ...
                        for _ in range(branches_per_idx):
                            ...
                            branches_created += 1
                        if branches_created >= remaining_slots: # 额度到了，退出
                            break


                # print(f"batch_size_in_line_525: {batch_size}")
            ...
            for output_ids, result_mask in zip(output_sequences, output_result_masks):
                ...
                logger.debug(f"len(output_ids): {len(output_ids)}, len(result_mask): {len(result_mask)}")
        ...
        rollout_traces_key = []
        rollout_traces_value = []
        max_step_len = 10
        for index, one_rollout in rollout_traces.items():
            one_rollout_key_list = []
            one_rollout_value_list = []
            for one_rollout_one_step in one_rollout: # list
                one_rollout_key_list.append(one_rollout_one_step["key"])
                one_rollout_value_list.append(one_rollout_one_step["value"])
            while len(one_rollout_key_list) < max_step_len:
                one_rollout_key_list.append("null")
                one_rollout_value_list.append("null")
            rollout_traces_key.append(one_rollout_key_list)
            rollout_traces_value.append(one_rollout_value_list)


        non_tensor_batch["rollout_traces_key"] = np.array(rollout_traces_key, dtype=object)
        non_tensor_batch["rollout_traces_value"] = np.array(rollout_traces_value, dtype=object)
