...
class RequestMonitor:
    ...
    def get_stats(self):
        with self.lock:
            ...
            self.request_times = [t for t in self.request_times if current_time - t < 300]
...
def start_monitoring_thread():
    def monitor():
        while True:
            ...
            print(
                f"[Search] active_requests={stats['active_requests']} "
...
def load_corpus(corpus_path: str):
    ...
    with open(corpus_path, 'r') as f:
        for line in tqdm(f):
            corpus.append(json.loads(line.strip())['contents'])
    print('加载语料完成，语料长度为：', len(corpus))
    print('示例语料：', corpus[0])
    return corpus
...
class Config:
    ...
    def __init__(
        ...
        index_path: str = "./wiki/wiki18_MiniLM.index",
        corpus_path: str = "./wiki/wiki-18.jsonl",
        dataset_path: str = "./wiki",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--index_path", type=str, default="./wiki/wiki18_MiniLM.index", help="Corpus indexing file."
    )

    parser.add_argument(
        "--corpus_path",
        type=str,
        default="./wiki/wiki-18.jsonl",
