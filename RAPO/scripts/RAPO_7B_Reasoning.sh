...
# Set Python path
PATH_TO_ARPO="./RAPO"
export PYTHONPATH="${PATH_TO_ARPO}/verl_rapo_entropy:$PYTHONPATH"

# ============================ Basic Configuration ============================
# Experiment name and project
PROJECT_NAME="reasoning_tasks" # Modify experiment group
EXPERIMENT_NAME="RAPO_reasoning_tasks" # Modify experiment name

# Configuration file path
CONFIG_PATH="${PATH_TO_ARPO}/scripts/config" # Modify the absolute path of the config folder, relative path is not recommended
CONFIG_NAME="ppo_trainer.yaml"

# Distributed training settings
NNODES=1                            
N_GPUS_PER_NODE=8
export CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 # 0,1,2,3  # List of visible GPUs
...
PPO_MINI_BATCH_SIZE=16              # PPO mini-batch size # 16
MAX_PROMPT_LENGTH=1536              # Maximum prompt length
MAX_RESPONSE_LENGTH=4096            # Maximum response length

# Data file paths
TRAIN_FILES="${PATH_TO_ARPO}/rl_datasets/train_10k.parquet" # Modify training data path
VALID_FILES="${PATH_TO_ARPO}/rl_datasets/valid.parquet" # Modify validation data path
...
CUSTOM_REWARD_FUNCTION_PATH="${PATH_TO_ARPO}/verl_rapo_entropy/verl/utils/reward_score/deep_research.py" # Modify reward function path
...
# ============================ Path Configuration ============================
# Save path
SAVE_PATH="${PATH_TO_ARPO}/rl_output/${EXPERIMENT_NAME}" # Modify save path
...
python3 -m verl.trainer.main_ppo \
    ...
    actor_rollout_ref.rollout.no_retrieve_first_k=${no_retrieve_first_k} \
    actor_rollout_ref.rollout.retrieve_prob=${retrieve_prob} \
    actor_rollout_ref.rollout.shaping_weight=${shaping_weight} \
    actor_rollout_ref.rollout.advantage_weight=${advantage_weight} \
