...
class vLLMRolloutWithTools(vLLMRollout):
    ...
    def __init__(self, model_path: str, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        self.beam_size = self.config.get("beam_size", 1) # beam_size = 2
        self.branch_probability = self.config.get("branch_probability", 0.5) # branch prob = 0.5
        self.entropy_weight = self.config.get("entropy_weight", 0.5)

        self.no_retrieve_first_k = self.config.get("no_retrieve_first_k", 8)
        self.retrieve_prob = self.config.get("retrieve_prob", 0.5)

        # 从配置中获取工具设置
        tools_config = self.config.get("tools", OmegaConf.create({}))

        # 获取工具通用配置
        self.tool_call_limit = tools_config.get("call_limit", 5) # 3次
        self.max_tool_workers = tools_config.get("max_workers", 64) # 64
        self.tool_timeout = tools_config.get("timeout", 120) # 120s工具调用超时

        # 其他可能的工具通用配置
        self.tool_retry_count = tools_config.get("retry_count", 3) # 3次
        self.tool_verbose_logging = tools_config.get("verbose_logging", False) # True, 启用详细日志
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        tool_metrics = {
            ...
            "retrieve/total_calls": 0,
            "retrieve/successful_calls": 0,
            "retrieve/failed_calls": 0,
            "retrieve/total_execution_time": 0.0,
            "retrieve/avg_execution_time": 0.0,
            "retrieve/max_execution_time": 0.0,
            "retrieve/max_retries": 0,
            "retrieve/total_retries": 0,
            "retrieve/call_limit_reached_count": 0,
        }
        ...
        with self.update_sampling_params(**kwargs):
            num_samples = self.sampling_params.n # 16
            ...
            initial_rollouts = self.initial_rollouts # 8
            ...
            retrieve_counters = []
            step_retrieve_entropy = []
            step_retrieve_token_lengths = []
            step_retrieve_pad_num = 20
            ...
            for i, ids in enumerate(prompt_token_ids_list):
                for _ in range(initial_rollouts):
                    ...
                    retrieve_counters.append(0)
                    step_retrieve_entropy.append([])
                    step_retrieve_token_lengths.append([])
            ...
            max_len = self.config.response_length # 4096

            while active_indices:
                active_prompts = [curr_inputs[i] for i in active_indices] # token id
                ...
                for i, out_idx in enumerate(active_indices):
                    ...
                    step_retrieve_entropy[out_idx].append(entropy)
                    step_retrieve_token_lengths[out_idx].append(0)
                    if out_idx not in self.initial_entropy_dict: # 记录初试熵
                ...
                for i, out_idx in enumerate(active_indices):
                    ...
                    if is_tool_call:
                        ...
                        if call_counters[out_idx] < self.tool_call_limit:
                            ...
                            curr_inputs[out_idx].append(eos_token_id) # 直接结束该样本
                ...
                # {"search": [{dix: "content"}, {idx: "content"}], "python": [{...}, {...}]}
                if any(tool_requests.values()):
                    ...
                    for tag, requests in tool_requests.items():
                        ...
                        tool = self.tools[tag]
                        for req in requests: # {idx: "content"}
                ...
                # Apply retrieve: retrieve the generated texts in an off-policy manner
                retrieve_idx_list = []
                for i, active_idx in enumerate(final_active_indices):
                    if active_idx % initial_rollouts > self.no_retrieve_first_k:
                        if random.random() < self.retrieve_prob:
                            retrieve_counters[active_idx] += 1
                            retrieve_key = self.tokenizer.decode(curr_inputs[active_idx])
                            retrieve_idx_list.append({"index": active_idx, "content": retrieve_key})
                            tool_metrics["retrieve/total_calls"] += 1


                if len(retrieve_idx_list):
                    logger.info(f"[Retrieve] Processing retrieve for {len(retrieve_idx_list)} total requests")
                    futures = {}
                    tool = self.tools["retrieve"]
                    for req in retrieve_idx_list:
                        logger.debug(f"[Retrieve] Submitting retrieve request: idx={req['index']}")
                        future = self.executor.submit(self._execute_tool_with_retry, tool, req["content"])
                        futures[future] = {"index": req["index"], "tag": "retrieve"}

                    total_futures = len(futures)
                    completed_futures = 0
                    logger.debug(f"[Retrieve] Submitted {total_futures} retrieve requests for execution")
                    for future in concurrent.futures.as_completed(futures):
                        completed_futures += 1
                        fut_info = futures[future]
                        idx = fut_info["index"]
                        ...
                        try:
                            ...
                            if success:
                                tool_metrics["retrieve/successful_calls"] += 1
                                success_per_tool[tag] += 1
                                logger.info(
                                    f"[Retrieve] Retrieve for sample {idx} completed successfully in {execution_time:.2f}s, result length: {len(result_text)}")
                            else:
                                tool_metrics["retrieve/failed_calls"] += 1
                                result_text = f"[Retrieve] returned empty output."
                                logger.warning(f"[Retrieve] Retrieve for sample {idx} failed after {retry_count} retries, execution time: {execution_time:.2f}s")

                            tool_metrics["retrieve/total_execution_time"] += execution_time
                            tool_metrics["retrieve/max_execution_time"] = max(tool_metrics["retrieve/max_execution_time"], execution_time)
                            tool_metrics["retrieve/total_retries"] += retry_count
                            tool_metrics["retrieve/max_retries"] = max(tool_metrics["retrieve/max_retries"], retry_count)

                            total_time_per_tool[tag] += execution_time

                            if not result_text:
                                result_text = f"[Retrieve] Retrieve returned empty output."
                                logger.warning(f"[Retrieve] Retrieve for sample {idx} returned empty output, execution time: {execution_time:.2f}s")
                            else:
                                logger.debug(f"[Retrieve] Tool({tag}) result: {result_text}")

                        except Exception as e:
                            logger.error(f"[Retrieve] Retrieve execution failed for sample {idx}: {e}")
                            result_text = f"Error: Retrieve execution failed with message: {e}"
                            tool_metrics["retrieve/failed_calls"] += 1


                        formatted_result = f" Additional context is enclosed in <retrieve> </retrieve> tags. Use it if helpful. <retrieve>\n{result_text}\n</retrieve>"
                        result_tokens = self.tokenizer.encode(formatted_result)
                        logger.debug(f"[Retrieve] Result for retrieve, sample {idx} tokenized to {len(result_tokens)} tokens")
                        if len(curr_inputs[idx]) + len(result_tokens) < max_len:
                            curr_inputs[idx].extend(result_tokens)
                            result_masks[idx].extend([0] * len(result_tokens))
                            step_retrieve_entropy[idx].append(-1.0)
                            step_retrieve_token_lengths[idx].append(len(result_tokens))
            ...
            # shape [batch_size * num_samples, response_length] result mask
            ...
            final_batch_size = input_ids.size(0) # batch_size * num_samples
            ...
            response_attention_mask = get_response_mask(response_id=response, eos_token=eos_token_id, dtype=attention_mask.dtype) # eos之后的部分置0
            ...
            if tool_metrics["retrieve/total_calls"] > 0:
                tool_metrics["retrieve/avg_execution_time"] = tool_metrics["retrieve/total_execution_time"] / tool_metrics["retrieve/total_calls"]
            ...
            total_rollout_num = len(step_retrieve_entropy)
            padded_step_retrieve_entropy = np.full((total_rollout_num, step_retrieve_pad_num), -2.0, dtype=np.float32)
            padded_step_retrieve_token_lengths = np.full((total_rollout_num, step_retrieve_pad_num), -2.0, dtype=np.float32)
            padded_step_mask = np.zeros((total_rollout_num, step_retrieve_pad_num), dtype=np.int32)

            for i in range(total_rollout_num):
                steps = len(step_retrieve_entropy[i][:step_retrieve_pad_num])
                if steps == 0:
                    continue
                padded_step_retrieve_entropy[i, :steps] = step_retrieve_entropy[i][:step_retrieve_pad_num]
                padded_step_retrieve_token_lengths[i, :steps] = step_retrieve_token_lengths[i][:step_retrieve_pad_num]
                padded_step_mask[i, :steps] = 1

            batch["padded_step_retrieve_entropy"] = torch.from_numpy(padded_step_retrieve_entropy)
            batch["padded_step_retrieve_token_lengths"] = torch.from_numpy(padded_step_retrieve_token_lengths)
            batch["padded_step_mask"] = torch.from_numpy(padded_step_mask)
