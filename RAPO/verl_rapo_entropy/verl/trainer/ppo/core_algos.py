...
def compute_grpo_outcome_advantage(
        token_level_rewards: torch.Tensor,
        retrieve_rewards: torch.Tensor,
        retrieve_gates: torch.Tensor,
        response_mask: torch.Tensor,
        index: np.ndarray,
        epsilon: float = 1e-6,
    ...
    Returns:
        ...
        Returns: `(torch.Tensor)`
            shape is (bs, response_length)
    """
    # ========= 1. outcome reward -> acc advantage =========
    scores = token_level_rewards.sum(dim=-1)
    # ========= 2. retrieve reward -> retrieve advantage =========
    retrieve_scores = retrieve_rewards.clone()

    id2score, id2score_retrieve = defaultdict(list), defaultdict(list)
    id2mean, id2mean_retrieve = {}, {}
    id2std, id2std_retrieve = {}, {}
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            ...
            # if retrieve_scores[i] > 0:
            id2score_retrieve[index[i]].append(retrieve_scores[i])
        # print(f"id2score: {id2score}, id2score_retrieve: {id2score_retrieve}")
        ...
        for idx in id2score:
            if len(id2score[idx]) == 1:
                ...
                id2mean_retrieve[idx] = torch.tensor(0.0)
                id2std_retrieve[idx] = torch.tensor(1.0)
            elif len(id2score[idx]) > 1:
                id2mean[idx] = torch.mean(torch.tensor(id2score[idx]))
                id2std[idx] = torch.std(torch.tensor([id2score[idx]]))
                # if len(id2score_retrieve[idx]) > 1:
                id2mean_retrieve[idx] = torch.mean(torch.tensor(id2score_retrieve[idx]))
                id2std_retrieve[idx] = torch.std(torch.tensor([id2score_retrieve[idx]]))
                # elif len(id2score_retrieve[idx]) == 1:
                #     id2mean_retrieve[idx] = torch.tensor(0.0)
                #     id2std_retrieve[idx] = torch.tensor(1.0)
            else:
        ...
        for i in range(bsz):
            if norm_adv_by_std_in_grpo:
                ...
                # if retrieve_scores[i] > 0:
                retrieve_scores[i] = (retrieve_scores[i] - id2mean_retrieve[index[i]]) / (
                            id2std_retrieve[index[i]] + epsilon)
            else:
                scores[i] = scores[i] - id2mean[index[i]]
                # if retrieve_scores[i] > 0:
                retrieve_scores[i] = retrieve_scores[i] - id2mean_retrieve[index[i]]
        # Fuse two types of advantages
        # gated_retrieve_scores = retrieve_gates * retrieve_scores
        final_scores = scores * (1.0 + 0.2 * retrieve_scores)

        final_scores = final_scores.unsqueeze(-1) * response_mask
...
def compute_policy_loss(
    ...
    alpha = 1
    token_level_retrieve_token_ratios = retrieve_token_ratios.unsqueeze(-1) * response_mask  # (bs, L)
    ratio = (1.0 + 0.05 * torch.pow(token_level_retrieve_token_ratios, alpha)) * ratio
